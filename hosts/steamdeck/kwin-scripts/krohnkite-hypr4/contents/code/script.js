...
class KWinConfig {
    constructor() {
        ...
        [
            ...
            ["dwindleLayoutOrder", 0, DwindleLayout],
...
class TilingController {
    ...
    onWindowAdded(ctx, window) {
        ...
        const focused = ctx.currentWindow;
        if (focused && focused !== window && window.tileable) {
            this.engine.windows.move(window, focused, true);
        }
        if (window.tileable) {
            const srf = ctx.currentSurface;
...
class TilingEngine {
    ...
    enforceSize(ctx, window) {
        if (window.tiled && !window.actualGeometry.equals(window.geometry)) {
            const sig = String(window.geometry);
            if (window._enforceSig === sig) {
                if (window._enforceCount >= 3)
                    return;
                window._enforceCount += 1;
            }
            else {
                window._enforceSig = sig;
                window._enforceCount = 1;
            }
            ctx.setTimeout(() => {
                if (window.tiled)
...
class SpiralLayout {
    ...
    bore(depth) {
        if (this.depth === depth)
            return;
        const root = new HalfSplitLayoutPart(new FillLayoutPart(), new FillLayoutPart());
        root.angle = 0;
        let hpart = root;
        for (let i = 1; i < depth - 1; i++) {
            const npart = new HalfSplitLayoutPart(new FillLayoutPart(), new FillLayoutPart());
            npart.angle = (i % 2) * 90;
...
class DwindleLayout {
    ...
    constructor() {
        this.classID = DwindleLayout.id;
        this.root = null;
        DwindleLayout.instances = (DwindleLayout.instances || 0) + 1;
        this.inst = DwindleLayout.instances;
    }
    isLeaf(n) {
    ...
    collectLeaves(node, out) {
        ...
        if (this.isLeaf(node))
            ...
            this.collectLeaves(node.a, out);
            this.collectLeaves(node.b, out);
        }
    }
    removeId(node, id) {
        if (!node)
        ...
        node.a = this.removeId(node.a, id);
        node.b = this.removeId(node.b, id);
        if (!node.a)
            return node.b;
        if (!node.b)
            return node.a;
    ...
    findLeaf(node, id) {
        ...
        return this.findLeaf(node.a, id) || this.findLeaf(node.b, id);
    }
    insertAt(targetId, newId) {
        const walk = (node) => {
            if (this.isLeaf(node)) {
                if (node.id !== targetId)
                    return node;
                const r = node.rect;
                const horizontal = r ? r.width >= r.height : true;
                return {
    ...
    layoutRects(node, rect, gap) {
        ...
        if (node.horizontal) {
            ...
            ra = new Rect(rect.x, rect.y, aw, rect.height);
            rb = new Rect(rect.x + aw + gap, rect.y, rect.width - aw - gap, rect.height);
        }
        else {
            const ah = Math.round((rect.height - gap) * ratio);
            ra = new Rect(rect.x, rect.y, rect.width, ah);
            rb = new Rect(rect.x, rect.y + ah + gap, rect.width, rect.height - ah - gap);
        }
        this.layoutRects(node.a, ra, gap);
        this.layoutRects(node.b, rb, gap);
    }
    pathTo(id) {
        const path = [];
        const walk = (node) => {
            if (this.isLeaf(node))
                return node.id === id;
            if (walk(node.a)) {
                path.push({ node: node, side: "a" });
                return true;
            }
            if (walk(node.b)) {
                path.push({ node: node, side: "b" });
                return true;
            }
            return false;
        };
    ...
    apply(ctx, tileables, area, gap) {
        tileables.forEach((tileable) => (tileable.state = WindowState.Tiled));
        const present = {};
        tileables.forEach((t) => (present[t.id] = t));
        const srf = ctx.drvctx ? ctx.drvctx.currentSurface : null;
        const leaves = [];
        this.collectLeaves(this.root, leaves);
        leaves.forEach((leaf) => {
            if (present[leaf.id])
                return;
            const w = ctx.engine.windows.getWindowById(leaf.id);
            if (!w ||
                (srf && !w.visible(srf)) ||
                w.state === WindowState.Floating ||
                w.state === WindowState.Docked)
                this.root = this.removeId(this.root, leaf.id);
        });
        let target = null;
        let best = -1;
        for (const t of tileables) {
            if (this.findLeaf(this.root, t.id) &&
        ...
        for (const t of tileables) {
            ...
            if (!target) {
                const ls = [];
                this.collectLeaves(this.root, ls);
                target = ls[ls.length - 1].id;
            }
            this.layoutRects(this.root, area, gap);
            this.insertAt(target, t.id);
            target = t.id;
        }
        if (!this.root)
            return;
        this.layoutRects(this.root, area, gap);
        const out = [];
        this.collectLeaves(this.root, out);
        out.forEach((leaf) => {
            const win = present[leaf.id];
            if (win)
                win.geometry = leaf.rect;
        });
    }
    adjust(area, tiles, basis, delta, gap) {
        const path = this.pathTo(basis.id);
        if (!path)
            return;
        const adjustEdge = (horizontal, side, amount) => {
            if (!amount)
                return;
            for (const step of path) {
                const n = step.node;
                if (n.horizontal === horizontal && step.side === side && n.rect) {
                    const span = (horizontal ? n.rect.width : n.rect.height) - gap;
                    if (span <= 0)
                        return;
                    n.ratio = Math.min(0.9, Math.max(0.1, n.ratio + amount / span));
                    return;
                }
            }
        };
        adjustEdge(true, "a", delta.east);
        adjustEdge(true, "b", -delta.west);
        adjustEdge(false, "a", delta.south);
        adjustEdge(false, "b", -delta.north);
    }
    handleShortcut(ctx, input, data) {
        let dir = null;
        if (input === Shortcut.SwapUp || input === Shortcut.ShiftUp)
            dir = "up";
        else if (input === Shortcut.SwapDown || input === Shortcut.ShiftDown)
            dir = "down";
        else if (input === Shortcut.SwapLeft || input === Shortcut.ShiftLeft)
            dir = "left";
        else if (input === Shortcut.SwapRight || input === Shortcut.ShiftRight)
            dir = "right";
        if (dir !== null) {
            const win = ctx.currentWindow;
            if (!win)
                return true;
            const neighbor = ctx.engine.getNeighborByDirection(ctx.drvctx, win, dir);
            if (neighbor) {
                const la = this.findLeaf(this.root, win.id);
                const lb = this.findLeaf(this.root, neighbor.id);
                if (la && lb) {
                    const tmp = la.id;
                    la.id = lb.id;
                    lb.id = tmp;
                }
                ctx.engine.windows.swap(win, neighbor);
            }
            return true;
        }
        if (input === Shortcut.SetMaster) {
            const win = ctx.currentWindow;
            if (!win)
                return true;
            const ls = [];
        ...
        if (input === Shortcut.Rotate) {
            const win = ctx.currentWindow;
            const path = win ? this.pathTo(win.id) : null;
            if (path && path.length > 0)
                path[0].node.horizontal = !path[0].node.horizontal;
            return true;
        }
        return false;
    }
    clone() {
        return new DwindleLayout();
    }
    toString() {
        return "DwindleLayout()";
    }
}
DwindleLayout.id = "DwindleLayout";
